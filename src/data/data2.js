export const menuData = {
  drinks: {
    category: "Drinks",
    path: "drinks",
    subCategory: [
      {
        name: "Hot Coffees",
        path: "hot-coffees",
        subSubCategory: [
          {
            name: "Brewed Coffee",
            items: [
              "Caffè Misto",
              "Featured Blonde Roast - True North Blend™",
              "Decaf Pike Place® Roast",
              "Featured Medium Roast - Pike Place® Roast",
              "Featured Dark Roast",
            ],
          },
          {
            name: "Americanos",

            items: [
              "Brown Sugar Oat Americano",
              "Caffè Americano",
              "Americano Misto",
            ],
          },
          { name: "Cappuccino", items: ["Cappuccino"] },
          {
            name: "Espresso Shots",
            items: ["Espresso", "Espresso Con Panna"],
          },
          {
            name: "Flat Whites",
            items: ["Honey Almond Flat White", "Flat White"],
          },
          {
            name: "Lattes",
            items: [
              "Pumpkin Spice Latte",
              "Oat Latte",
              "Starbucks Blonde® Hazelnut Latte",
              "Starbucks Blonde® Vanilla Latte",
              "Caffè Latte",
              "Cinnamon Dolce Latte",
            ],
          },
          {
            name: "Macchiatos",
            items: [
              "Apple Crisp Oat Macchiato",
              "Caramel Macchiato",
              "Espresso Macchiato",
            ],
          },
          {
            name: "Mochas",
            items: ["Caffè Mocha", "White Chocolate Mocha"],
          },
          {
            name: "Clover Brewed® Coffees",
            items: [
              "Clover® Starbucks Reserve® Rwanda Sholi",
              "Clover® Starbucks Reserve® Papua New Guinea Moanti",
            ],
          },
          {
            name: "Coffee Travellers",
            items: [
              "Coffee Traveller - Starbucks Blonde® Roast",
              "Coffee Traveller - Pike Place® Roast",
              "Coffee Traveller - Decaf Pike Place® Roast",
              "Coffee Traveller - Dark Roast",
            ],
          },
        ],
      },
      {
        name: "Hot Teas",
        path: "hot-teas",
        subSubCategory: [
          {
            name: "Chai Teas",
            items: ["Chai Tea Latte", "Teavana® Chai Tea"],
          },
          {
            name: "Black Teas",
            items: [
              "London Fog Tea Latte",
              "Earl Grey Tea",
              "Royal English Breakfast Tea",
              "Royal English Breakfast Tea Latte",
            ],
          },
          {
            name: "Green Teas",
            items: [
              "Matcha Tea Latte",
              "Emperor’s Clouds & Mist®",
              "Honey Citrus Mint Tea",
              "Jade Citrus Mint® Brewed Tea",
            ],
          },
          {
            name: "Herbal Teas",
            items: ["Mint Majesty®", "Peach Tranquility®"],
          },
        ],
      },
      {
        name: "Hot Drinks",
        path: "hot-drinks",
        subSubCategory: [
          {
            name: "Hot Chocolates",
            items: ["Hot Chocolate", "White Hot Chocolate"],
          },
          {
            name: "Steamed Juice",
            items: ["Steamed Apple Juice", "Caramel Apple Spice"],
          },
          {
            name: "Steamers",
            items: ["Pumpkin Spice Crème", "Steamed Milk", "Vanilla Steamer"],
          },
        ],
      },
      {
        name: "Frappuccino® Blended Beverages",
        path: "frappuccino-blended-beverages",
        subSubCategory: [
          {
            name: "Coffee Frappuccino®",
            items: [
              "Apple Crisp Oat Frappuccino® Blended Beverage",
              "Pumpkin Spice Frappuccino® Blended Beverage",
              "White Chocolate Mocha Frappuccino® Blended Beverage",
              "Mocha Cookie Crumble Frappuccino® Blended Beverage",
              "Caramel Ribbon Crunch Frappuccino® Blended Beverage",
              "Caramel Frappuccino® Blended Beverage",
              "Espresso Frappuccino® Blended Beverage",
              "Coffee Frappuccino® Blended Beverage",
              "Java Chip Frappuccino® Blended Beverage",
              "Mocha Frappuccino® Blended Beverage",
              "Caffè Vanilla Frappuccino® Blended Beverage",
            ],
          },
          {
            name: "Crème Frappuccino®",
            items: [
              "Apple Crisp Oat Crème Frappuccino® Blended Beverage",
              "Pumpkin Spice Crème Frappuccino® Blended Beverage",
              "Caramel Ribbon Crunch Crème Frappuccino® Blended Beverage",
              "White Chocolate Crème Frappuccino® Blended Beverage",
              "Chocolate Cookie Crumble Crème Frappuccino®",
              "Strawberry Crème Frappuccino® Blended Beverage",
              "Vanilla Bean Crème Frappuccino® Blended Crème",
              "Matcha Crème Frappuccino® Blended Beverage",
              "Chai Crème Frappuccino® Blended Beverage",
              "Double Chocolaty Chip Crème Frappuccino® Blended Beverage",
            ],
          },
        ],
      },
      {
        name: "Cold Coffees",
        path: "cold-coffees",
        subSubCategory: [
          {
            name: "Cold Brews",
            items: [
              "Pumpkin Cream Cold Brew",
              "Cinnamon Caramel Cream Cold Brew",
              "Chocolate Cream Cold Brew",
              "Salted Caramel Cream Cold Brew",
              "Vanilla Sweet Cream Cold Brew",
              "Starbucks® Cold Brew Coffee",
            ],
          },
          {
            name: "Iced Americano",
            items: ["Iced Caffè Americano"],
          },
          {
            name: "Nitro Cold Brews",
            items: [
              "Cinnamon Caramel Cream Nitro Cold Brew",
              "Nitro Cold Brew",
              "Vanilla Sweet Cream Nitro Cold Brew",
            ],
          },
          {
            name: "Iced Coffees",
            items: ["Iced Coffee", "Iced Coffee with Milk", "Iced Espresso"],
          },
          {
            name: "Iced Shaken Espresso",
            items: [
              "Iced Apple Crisp Oat Shaken Espresso",
              "Iced Toasted Vanilla Oat Shaken Espresso",
              "Iced Brown Sugar Oat Shaken Espresso",
              "Iced Shaken Espresso",
            ],
          },
          {
            name: "Iced Flat Whites",
            items: ["Iced Honey Almond Flat White", "Iced Flat White"],
          },
          {
            name: "Iced Lattes",
            items: [
              "Iced Pumpkin Spice Latte",
              "Iced Oat Latte",
              "Iced Caffè Latte",
              "Iced Starbucks Blonde® Vanilla Latte",
              "Iced Cinnamon Dolce Latte",
              "Iced Starbucks Blonde® Hazelnut Latte",
            ],
          },
          {
            name: "Iced Macchiatos",
            items: ["Iced Apple Crisp Oat Macchiato", "Iced Caramel Macchiato"],
          },
          {
            name: "Iced Mochas",
            items: ["Iced Caffè Mocha", "Iced White Chocolate Mocha"],
          },
        ],
      },
      {
        name: "Iced Teas",
        path: "iced-teas",
        subSubCategory: [
          {
            name: "Iced Chai Teas",
            items: ["Iced Pumpkin Cream Chai Tea Latte", "Iced Chai Tea Latte"],
          },
          {
            name: "Iced Black Teas",
            items: [
              "Iced Black Tea",
              "Iced Black Tea Lemonade",
              "Iced London Fog Tea Latte",
              "Iced Royal English Breakfast Tea Latte",
            ],
          },
          {
            name: "Iced Green Teas",
            items: [
              "Iced Matcha Tea Latte",
              "Matcha Tea Lemonade",
              "Iced Peach Green Tea",
              "Iced Peach Green Tea Lemonade",
              "Iced Green Tea Lemonade",
              "Iced Green Tea",
            ],
          },
          {
            name: "Iced Herbal Teas",
            items: [
              "Iced Passion Tango™ Tea",
              "Iced Passion Tango® Tea Lemonade",
            ],
          },
        ],
      },
      {
        name: "Cold Drinks",
        path: "cold-drinks",
        subSubCategory: [
          {
            name: "Starbucks Refreshers®",
            items: [
              "Frozen Pineapple Passionfruit Lemonade Starbucks Refreshers® Beverage",
              "Frozen Strawberry Açaí Lemonade Starbucks Refreshers® Beverage",
              "Frozen Mango Dragonfruit Lemonade Starbucks Refreshers® Beverage",
              "Dragon DrinkTM/MC Starbucks Refreshers® Beverage",
              "Mango Dragonfruit Starbucks Refreshers® Beverage",
              "Mango Dragonfruit Lemonade Starbucks Refreshers® Beverage",
              "Paradise Drink Starbucks Refreshers® Beverage",
              "Pineapple Passionfruit Starbucks Refreshers® Beverage",
              "Pineapple Passionfruit Lemonade Starbucks Refreshers® Beverage",
              "Strawberry Coconut Drink",
              "Strawberry Açaí Lemonade Starbucks Refreshers® Beverage",
              "Strawberry Açaí Starbucks Refreshers® Beverage",
            ],
          },
          {
            name: "Milk",
            items: [
              "Cold Milk",
              "Milk2Go 1% Chocolate Milk",
              "Milk2Go Chocolate Protein Shake",
              "Milk2Go 2% Partly Skimmed",
            ],
          },
          {
            name: "Sparkling Water",
            items: [
              "San Pellegrino® Essenza Dark Tangerine & Wild Strawberry",
              "San Pellegrino® Essenza Dark Morello Cherry & Pomegranate",
            ],
          },
          {
            name: "Water",
            items: ["Flow® Original Water", "Flow® Strawberry & Rose Water"],
          },
          {
            name: "Juice",
            items: [
              "Kiju® Organic Apple Juice",
              "RISE Kombucha Fresh Ginger",
              "RISE Kombucha Peach & Mango",
              "Well OJ Cold Pressed Juice",
              "Well Greens Cold Pressed Juice",
              "Be Well Cold Pressed Juice",
              "San Pellegrino® Blood Orange Sparkling Fruit Beverage",
              "San Pellegrino® Limonata Beverage",
              "Lemonade",
              "Blended Strawberry Lemonade",
            ],
          },
        ],
      },
    ],
  },
  food: {
    category: "Food",
    path: "food",
    subCategory: [
      {
        name: "Hot Breakfast",
        path: "hot-breakfast",
        subSubCategory: [
          {
            name: "Breakfast Sandwiches",
            items: [
              "Beyond Meat®, Cheddar & Egg Sandwich",
              "Everything Croissant & Roasted Ham Sandwich",
              "Bacon, Gouda & Egg Sandwich",
              "Bacon-Style Turkey, Cheddar & Egg White Sandwich",
              "Double-Smoked Bacon, Cheddar & Egg Sandwich",
              "Sausage, Cheddar & Egg Sandwich",
            ],
          },
          {
            name: "Breakfast Wraps",
            items: [
              "Bacon, Sausage & Egg Wrap",
              "Southwest Potato, Black Bean & Egg Wrap",
              "Spinach, Feta and Cage Free Egg White Wrap",
            ],
          },
          {
            name: "Sous Vide Egg Bites & Bakes",
            items: [
              "Bacon & Gruyère Egg Bites",
              "Egg White & Roasted Red Pepper Egg Bites",
              "Potato, Cheddar & Chive Bakes",
            ],
          },
        ],
      },
      {
        name: "Bakery",
        path: "bakery",
        subSubCategory: [
          { name: "Bagels", items: ["Everything Bagel"] },
          {
            name: "Cake Pops",
            items: [
              "Cookies & Cream Cake Pop",
              "Chocolate Cake Pop",
              "Birthday Cake Pop",
            ],
          },
          {
            name: "Cookies & Brownies",
            items: [
              "Fox Sugar Cookie",
              "Double Chocolate Brownie",
              "Chocolate Chunk Cookie",
              "Ginger Molasses Cookie",
            ],
          },
          {
            name: "Croissants",
            items: ["Cheese Danish", "Butter Croissant", "Chocolate Croissant"],
          },
          {
            name: "Dessert Bars",
            items: ["Gluten-Free Marshmallow Dream Bar", "Oat Bar"],
          },
          {
            name: "Loaves & Cakes",
            items: [
              "Pumpkin & Pepita Loaf",
              "Iced Lemon Loaf",
              "Banana Loaf",
              "Cinnamon Coffee Cake",
            ],
          },
          {
            name: "Muffins & Scones ",
            items: [
              "Pumpkin Cream Cheese Muffin",
              "Blueberry Muffin with Yogurt & Honey",
              "Petite Vanilla Bean Scone",
            ],
          },
        ],
      },
      {
        name: "Lunch",
        path: "lunch",
        subSubCategory: [
          {
            name: "Sandwiches and Wraps",
            items: [
              "Ham & Swiss on Baguette",
              "Chicken & Bacon on Brioche",
              "Crispy Grilled Cheese on Sourdough",
              "Tomato & Mozzarella on Focaccia",
              "Turkey, Provolone & Pesto on Ciabatta",
            ],
          },
          {
            name: "Snack Boxes",
            items: [
              "Original Hummus & Crackers Snack Box",
              "Artisan Charcuterie Snack Box",
              "Apples, PB & Trail Mix Snack Box",
            ],
          },
          {
            name: "Spreads",
            items: ["Avocado Spread"],
          },
        ],
      },
      {
        name: "Snacks & Sweets",
        path: "snacks-sweets",
        subSubCategory: [
          { name: "Biscotti & Cookies", items: ["Madeleines"] },
          {
            name: "Chocolates & Candy",
            items: [
              "Starbucks® Chocolate Covered Espresso Beans",
              "Squirrel Brand® - Crème Brûlée Almonds®",
              "Salted Almond Chocolate Bites",
              "Justin’s® Dark Chocolate Peanut Butter Cups",
            ],
          },
          {
            name: "Gum & Mints",
            items: ["Spearmint Gum"],
          },
          {
            name: "Popcorn & Chips",
            items: [
              "Butter Gourmet Popcorn",
              "Crispy Apple Chips",
              "Perfectly Salted Kettle Potato Chips",
            ],
          },
          { name: "Meat & Cheese", items: ["Marble Cheese Stick"] },
          {
            name: "Snack Bars",
            items: [
              "KIND® Nuts Cherry & Dark Chocolate Bar",
              "KIND Peanut Butter Dark Chocolate Bar",
              "Squirrel™ - Classic Almonds",
              "Squirrel Brand® - Crème Brûlée Almonds®",
            ],
          },
        ],
      },
      {
        name: "Oatmeal & Yogurt",
        path: "oatmeal-yogurt",
        subSubCategory: [
          {
            name: "Oatmeal & Yogurt",
            items: ["Rolled Steel-Cut Oatmeal"],
          },
        ],
      },
    ],
  },
  "at-home": {
    category: "At Home",
    path: "at-home",
    subCategory: [
      {
        name: "Whole Beam",
        path: "whole-beam",
        subSubCategory: [
          {
            name: "Starbucks Reserve®",
            items: [
              "Malawi Sable Farms Starbucks Reserve® Whole Bean Coffee",
              "Starbucks Reserve® Vietnam Da Lat",
              "Starbucks Reserve® Rwanda Sholi",
            ],
          },
          {
            name: "Blonde Roast",
            items: [
              "Starbucks® Green Apron Blend™",
              "Starbucks True North Blend®",
              "Starbucks Blonde® Espresso Roast",
            ],
          },
          {
            name: "Medium Roast",
            items: [
              "Guatemala Casi Cielo® Whole Bean Coffee",
              "Decaf Pike Place Roast",
              "Siren’s Blend®",
              "Pike Place® Roast",
            ],
          },
          {
            name: "Dark Roast",
            items: [
              "Starbucks Thanksgiving Blend®",
              "Gold Coast Blend®",
              "Komodo Dragon Blend®",
              "Caffè Verona®",
              "Sumatra",
              "Italian Roast",
              "Espresso Roast",
              "Decaf Espresso Roast",
            ],
          },
        ],
      },
      {
        name: "Teavana Tea",
        path: "teavana-tea",
        subSubCategory: [
          {
            name: "Teavana Tea",
            items: [
              "Teavana® Earl Grey",
              "Teavana® Emperor's Clouds & Mist®",
              "Teavana® Jade Citrus Mint®",
              "Teavana® Peach Tranquility®",
              "Teavana® Royal English Breakfast",
              "Teavana® Chai",
            ],
          },
        ],
      },
      {
        name: "VIA® Instant",
        path: "via-instant",
        subSubCategory: [
          {
            name: "Blonde Roast",
            items: ["VIA® True North Blend®"],
          },
          {
            name: "Medium Roast",
            items: ["Starbucks VIA® Instant Pike Place® Roast"],
          },
          {
            name: "Dark Roast",
            items: ["VIA® Italian Roast", "VIA® Decaf Italian Roast"],
          },
        ],
      },
    ],
  },
  merchanise: {
    category: "Merchandise",
    path: "merchandise",
    subCategory: [
      {
        name: "Cold Cups",
        path: "cold-cups",
        subSubCategory: [
          {
            name: "Cold Cups",
            items: [
              "Dichroic Bling Plastic Cold Cup - 710 mL",
              "Siren Shell Plastic Cold Cup - 710 mL",
              "Green Soft Touch Stainless-Steel Cold Cup - 710 mL",
              "Recycled Glass Cold Cup - 473 mL",
              "Cold To-Go Cup - 710 mL",
              "Siren Logo Plastic Cold Cup - 709 mL",
            ],
          },
        ],
      },
      {
        name: "Tumblers",
        path: "tumblers",
        subSubCategory: [
          {
            name: "Tumblers",
            items: [
              "Siren Logo Plastic Hot Cup - 16 fl oz",
              "Copper Pin Stainless-Steel Tumbler - 591 mL",
              "Drink-Thru Band Stainless-Steel Tumbler - 473 mL",
              "Recycled Plastic Tumbler - 473 mL",
              "Siren Logo Double-Wall Ceramic Tumbler - 354mL",
            ],
          },
        ],
      },
      {
        name: "Mugs",
        path: "mugs",
        subSubCategory: [
          {
            name: "Mugs",
            items: [
              "Pleat Desktop Ceramic Mug - 414 mL",
              "Glass Mug - 473 mL",
              "Recycled Ceramic Mug - 473 mL",
            ],
          },
        ],
      },
      {
        name: "Other",
        path: "other",
        subSubCategory: [
          {
            name: "Other",
            items: ["French Press", "Pour Over", "Small Shopping Bag"],
          },
        ],
      },
    ],
  },
};
